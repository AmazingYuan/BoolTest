...
void CombinatorialTest::AETG(const int candidateNum)
{
	...
	while (UncoverIsNotNULL())//算法的开始条件
	{
		...
		for (int i = 0; i < candidateNum;++i)//初始化一下这个候选测试集
		{
			...
			{
				testcase[j] =5/* rand() % 2*/;//给里面赋值随机的0或1，以防后患
			}
		...
		for (int M = 0; M < candidateNum; ++M)//选出candidateNum个候选测试用例，然后选出一个覆盖最多的
		{
				...
				*/
			{
				...
				std::vector<std::vector<std::map<std::vector<int>, std::vector<int>>::iterator>> readyToClearUncoverIters(2);
				...
				for (int j = 0; j < pickedSubscripts.size(); ++j)
				{
					...
					bool positionOfAfterPickedSubscripts = 0;
					if (pickedF < pickedSubscripts[j])//下标需要小的在前大的在后，不然会找不到
					{
						...
						positionOfAfterPickedSubscripts = 1;
					}
					//开始寻找这个组合是否存在
					...
					if (CombiOfF != uncover.end())
					{
						...
						for (int k = 0; k != uncover.count(twoSubscripts); ++k,++CombiOfF)
						{
							...
							{
								readyToClearUncoverIters[CombiOfF->second[positionOfAfterPickedSubscripts]].push_back(CombiOfF);
							}
						}
					}
					...
					//	}
					//	candidateTestCases[M][twoSubscripts[positionOfAfterPickedSubscripts]] ++;
					//}
					//else{
				...
				if (readyToClearUncoverIters[0].size() > readyToClearUncoverIters[1].size())
				{
					...
					candidateTestCases[M][pickedF] =0;
				}
				else if (readyToClearUncoverIters[0].size()==0&& readyToClearUncoverIters[1].size()==0){
					//这里应该选择一个最少使用的赋值
					int a0=0,a1 = 0;
					for (int m = 0; m < CoverArray.size(); ++m)
					{
						CoverArray[m][pickedF] ? ++a1 : ++a0;
					}
					a1>a0 ? candidateTestCases[M][pickedF] = 1 : candidateTestCases[M][pickedF] = 0;
				}
				else{
					//readyToClearUncoverIters[2] = readyToClearUncoverIters[1];//取1
					for (int x = 0; x < readyToClearUncoverIters[1].size(); ++x)
					...
					candidateTestCases[M][pickedF] = 1;
				}
